...
def load_resnet10_params(agent, image_keys=("image",), public=True):
    ...
    if not public:  # if github repo is not public, load from local file
        ...
        def _download_weights(target_path: str):
        ...
        # Check if the file exists and is readable; if corrupted, re-download.
        if os.path.exists(file_path):
            print(f"The ResNet-10 weights already exist at '{file_path}'.")
        else:
            _download_weights(file_path)

        try:
            with open(file_path, "rb") as f:
                encoder_params = pkl.load(f)
        except Exception as e:
            print(f"Failed to load '{file_path}' ({e}); re-downloading...")
            if os.path.exists(file_path):
                os.remove(file_path)
            _download_weights(file_path)
            with open(file_path, "rb") as f:
