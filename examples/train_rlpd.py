...
def actor(agent, data_store, intvn_data_store, env, sampling_rng):
    ...
    if FLAGS.eval_checkpoint_step:
        if FLAGS.checkpoint_path is None:
            raise ValueError("--checkpoint_path is required when --eval_checkpoint_step is set.")
        success_counter = 0
        time_list = []
    ...
    if FLAGS.checkpoint_path is None:
        ...
        pkl_files = glob.glob(os.path.join(FLAGS.checkpoint_path, "buffer/*.pkl"))
        if not pkl_files:
            start_step = 0
        else:
            latest_file = natsorted(pkl_files)[-1]
    ...
    for step in pbar:
        ...
        with timer.context("step_env"):
            episode_step += 1
            next_obs, reward, done, truncated, info = env.step(actions)
            manual_success = info.pop("manual_success", False)
            if "left" in info:
                info.pop("left")
            if "right" in info:
                info.pop("right")

            # Manual success override: press 'f' to mark current step as success
            if manual_success:
                reward = 1.0
                done = True
                TERMINAL_KEYBOARD_HUB.suspend()
                print(f"[Actor] Manual success at step {step}!")
            ...
            if step % 10 == 0:
                ...
                TERMINAL_KEYBOARD_HUB.suspend()
                print(f"[Actor] Step {step}, reward: {reward}, classifier_prob: {prob:.4f}, store: {len(data_store)}")
                TERMINAL_KEYBOARD_HUB.resume()
            # Send data to learner periodically so the learner sees fresher online data.
            if step > 0 and step % config.actor_update_period == 0:
                client.update()
            ...
            if done or truncated:
                done_reason = "truncated" if truncated else "unknown"
                if manual_success:
                    done_reason = "manual_success"
                elif reward:
                    done_reason = "reward_success"
                elif episode_step >= env.unwrapped.max_episode_length:
                    done_reason = "max_episode_length"
                info["episode"]["intervention_count"] = intervention_count
                info["episode"]["intervention_steps"] = intervention_steps
                info["episode"]["episode_step"] = episode_step
                info["episode"]["done_reason"] = done_reason
                stats = {"environment": info}  # send stats to the learner to log
                client.request("send-stats", stats)
                TERMINAL_KEYBOARD_HUB.suspend()
                pbar.set_description(f"last return: {running_return}")
                print(
                    f"[Actor] Episode done at global step {step}, "
                    f"episode_step: {episode_step}, reason: {done_reason}, "
                    f"total transitions: {len(transitions)}, calling client.update()"
                )
                TERMINAL_KEYBOARD_HUB.resume()
                running_return = 0.0
                intervention_count = 0
                ...
                client.update()
                TERMINAL_KEYBOARD_HUB.suspend()
                print(f"[Actor] client.update() completed")
                TERMINAL_KEYBOARD_HUB.resume()
                obs, _ = env.reset()
...
def learner(rng, agent, replay_buffer, demo_buffer, wandb_logger=None):
    ...
    while len(replay_buffer) < config.training_starts:
        ...
        if int(time.time()) % 5 == 0:
            print(f"[Learner] Waiting for data... replay_buffer size: {len(replay_buffer)}, demo_buffer size: {len(demo_buffer)}")
        time.sleep(1)
    pbar.update(len(replay_buffer) - pbar.n)  # Update progress bar
    pbar.close()
...
def main(_):
    ...
    if "render_mode" in inspect.signature(config.get_environment).parameters and not FLAGS.actor:
        ...
        for path in FLAGS.demo_path:
            with open(path, "rb") as f:
                ...
                for transition in transitions:
                    ...
                    # Flatten nested obs structure (state + images.{cam}) to flat (state + cam keys)
                    for obs_key in ["observations", "next_observations"]:
                        obs = transition[obs_key]
                        if "images" in obs:
                            for cam_key, cam_img in obs["images"].items():
                                obs[cam_key] = cam_img
                            obs.pop("images")
